...
report.py — Arquivos de saída do modo pressão (usados pela CLI e pela GUI).
    ...
    results.csv      θ [rad e °], P medida, P simulada, resíduo, P simulada
                     dos modelos reduzidos (k-Wiebe refinados a partir do
                     truncamento, k = N−1 … 1) e resíduos, Tg, x_b e
    ...
    plots/*.png      pressão medida × simulada, pressão × ângulo comparando
                     os modelos reduzidos (k-Wiebe refinados, k = N−1 … 1),
                     diagrama P–V (modelo × experimental), diagrama P–V da
                     comparação entre estágios, resíduo, fração queimada,
                     taxa de liberação de calor [kJ/rad] por estágio,
...
from ..core.parameters import Stage, stages_to_arrays
...
def _pressao_com_avaliador(theta, P1, stages, engine, Rc):
    """(P [kPa], avaliador) de um modelo: referência solve_ivp; se cair na
    fronteira de falha do passo adaptativo (RHS não física só para o
    integrador adaptativo), avalia com RK4 f64 em lote — o mesmo
    integrador da busca, validado contra a referência (≈ 2 kPa no ensaio).
    Levanta ODEFailure se nenhum dos dois integrar."""
    from .model import ODEFailure, batch_rk4, simulate
    try:
        P, _, _ = simulate(theta, P1, stages, engine, Rc)
        return P, "solve_ivp"
    except ODEFailure:
        p = {k: v[None, :] for k, v in stages_to_arrays(stages).items()}
        P = batch_rk4(theta, P1, np.array([float(Rc)]), p, engine)[0]
        if not np.all(np.isfinite(P)):
            raise ODEFailure("RK4 também falhou") from None
        return P, "rk4"


def _stage_comparisons(r, d) -> Dict[int, Dict]:
    """Modelos reduzidos {k: dict}, k = N−1 … 1, cada um com:

        P         P_sim [kPa] nos ângulos do ensaio
        rmse      RMSE do modelo k contra o ensaio [kPa]
        refinado  True = TRF a partir do truncamento; False = só truncamento
        avaliador "solve_ivp" | "rk4" (fronteira de falha do adaptativo)
        Rc, stages  parâmetros do modelo k (stages em rad)

    Um "k-Wiebe" de comparação parte dos k PRIMEIROS estágios do ajuste N
    com β renormalizado (Σβ = 1) e é REFINADO por mínimos quadrados (TRF) —
    o mesmo procedimento do candidato aninhado de ``compare_pressure`` —
    para que cada curva seja um k-Wiebe de fato ajustado ao ensaio, e não
    um truncamento arbitrário (que lembra uma curva não ajustada e pode
    ficar ACIMA do ajuste N, pois todo o calor queima na janela do 1º
    estágio). Se o refinamento falhar, usa-se o truncamento puro
    (``refinado = False``); se nenhum integrar, coluna NaN — o fato vira
    WARNING em warnings.txt (nunca silencioso). O refinamento é diagnóstico
    local a partir do ajuste N; não substitui a comparação completa com
    PSO (``--compare-stages`` / ``comparison.csv``).
    """
    from dataclasses import replace as dc_replace
    from .fit import PressureObjective, PressureParam, _refinar
    from .model import ODEFailure
    ...
    for k in range(len(r.stages) - 1, 0, -1):
        ...
        trunc = [Stage(s.beta / soma, s.theta0, s.duration, s.m, s.a)
                 for s in primeiros]
        candidatos = [(r.Rc, trunc, False)]      # fallback por último
        # refinamento TRF (mesma mecânica do candidato aninhado); exceção
        # ampla de propósito: qualquer falha cai no truncamento puro
        try:
            s_k = dc_replace(r.settings, n_stages=k)
            param_k = PressureParam(s_k)
            obj_k = PressureObjective(d, param_k, s_k)
            x0 = np.clip(param_k.encode(r.Rc, trunc), param_k.lower,
                         param_k.upper)
            Rc_ref, stages_ref = param_k.to_model(
                _refinar(obj_k, x0, param_k))
            candidatos.insert(0, (Rc_ref, stages_ref, True))
        except Exception:                                # noqa: BLE001
            pass
        P_k = np.full(d.n, np.nan)
        rmse, refinado, aval = float("nan"), False, "solve_ivp"
        Rc_k, stages_k = r.Rc, trunc
        for Rc_c, st_c, ref_c in candidatos:
            try:
                P_c, aval_c = _pressao_com_avaliador(
                    d.theta, float(d.P[0]), st_c, r.settings.engine, Rc_c)
            except ODEFailure:
                continue
            P_k = P_c
            rmse = float(np.sqrt(np.mean((P_c - d.P) ** 2)))
            refinado, aval = ref_c, aval_c
            Rc_k, stages_k = Rc_c, st_c
            break
        out[k] = {"P": P_k, "rmse": rmse, "refinado": refinado,
                  "avaliador": aval, "Rc": Rc_k, "stages": stages_k}
    return out
...
def write_outputs(outdir, r, d, comp: Optional[Dict] = None,
    ...
    (out / "warnings.txt").write_text(
        "".join(f"{w}\n" for w in
                list(r.warnings)
                + [f"WARNING: modelo reduzido {k}-Wiebe falhou (solve_ivp); "
                   f"colunas *_wiebe{k} em NaN."
                   for k, c in comp_est.items()
                   if not np.all(np.isfinite(c["P"]))]
                + [f"AVISO: {k}-Wiebe refinado avaliado com RK4 f64 — a "
                   f"referência solve_ivp falhou na fronteira de falha do "
                   f"passo adaptativo (candidato válido no RK4 da busca)."
                   for k, c in comp_est.items()
                   if c["refinado"] and c["avaliador"] == "rk4"]
                + [f"AVISO: refinamento do {k}-Wiebe não convergiu; usada "
                   f"a curva truncada (β renormalizado) sem refinamento."
                   for k, c in comp_est.items()
                   if not c["refinado"]
                   and np.all(np.isfinite(c["P"]))]),
        encoding="utf-8")
    from ..io.config import _plain
    payload = _plain(r.to_dict())
    if comp_est:
        payload["comparacao_truncada"] = {
            str(k): {"refinado": c["refinado"], "avaliador": c["avaliador"],
                     "rmse_kPa": c["rmse"], "Rc": c["Rc"],
                     "estagios_deg": [
                         {"beta": s.beta, "theta0_deg": math.degrees(s.theta0),
                          "duration_deg": math.degrees(s.duration),
                          "m": s.m, "a": s.a} for s in c["stages"]]}
            for k, c in comp_est.items()}
...
def _plots(pasta: Path, r, d, st_deg, comp_est: Optional[Dict] = None):
    ...
    from .engine import volume
    if comp_est:
        # pressão × ângulo e P–V com os modelos reduzidos (N−1 … 1 estágios,
        # refinados a partir do truncamento) + experimental
        ...
        def _rotulo(k, c):
            if not np.isfinite(c["rmse"]):
                return f"{k}-Wiebe (falhou)"
            estado = "refinado" if c["refinado"] else "truncado"
            if c["refinado"] and c["avaliador"] == "rk4":
                estado += ", RK4"
            return f"{k}-Wiebe ({estado}, RMSE {c['rmse']:.1f} kPa)"

        for i, k in enumerate(sorted(comp_est, reverse=True)):
            c = comp_est[k]
            ax.plot(thd, c["P"], lw=1.2,
                    color=cores[(i + 1) % len(cores)], label=_rotulo(k, c))
        ax.set(xlabel="θ [°]", ylabel="P [kPa]",
    ...
    if comp_est:
        ...
        for i, k in enumerate(sorted(comp_est, reverse=True)):
            c = comp_est[k]
            ax.plot(V, c["P"], lw=1.2, color=cores[(i + 1) % len(cores)],
                    label=f"{k}-Wiebe ({'refinado' if c['refinado'] else 'truncado'}, "
                          f"Rc {c['Rc']:.3f})")
